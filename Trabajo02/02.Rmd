...
Podemos comprobar que todo ha salido bien haciendo una gráfica de los primeros 10 polinomios de Legendre, que genera un resultado muy llamativo:
...
# Normalizamos
a_q <- a_q / sqrt(sum(sapply(seq(0,Qf), function(q){1 / (2*q + 1)})))

legendre.f <- function(x){
  res <- sapply(seq(0,Qf), function(q){ a_q[q+1] * legendre(q,x)})
...
legendre.g10.dat = t(sapply(legendre.X, function(x){ sapply(seq(2,10), function(n){x^n}) }))
...
Podemos ver cómo se ajustan ambas funciones a los datos:

```{r}
legendre.g2 <- function(x){
  sum(sapply(seq(1,2), function(i){ legendre.g2.pesos[i] * x^i})) + legendre.g2.pesos[3]
}

legendre.g10 <- function(x){
  sum(sapply(seq(1,9), function(i){ legendre.g10.pesos[i] * x^i})) + legendre.g10.pesos[10]
}

plot(legendre.X, legendre.Y, pch=20, col="blue")

x <- seq(-1, 1, by=0.01)
y <- sapply(x, legendre.g10)
points(x,y,type="l")
```
