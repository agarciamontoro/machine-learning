...
* `importance = TRUE` Parámetro de configuración para indicar que se evalúe la importancia de los predictores.
...
forest.best <- randomForest(medv ~ ., data = Boston, subset = trainIdx, ntrees=forest.best.N, importance = T)
```
...
El error de boosting con los parámetros fijados es mayor que el conseguido hasta ahora con *bagging* y *random forest*. Podemos intentar mejorar este error ajustando los parámetros con validación cruzada:
...
vemos que este árbol, aunque compacto, no es nada bueno. Lo primero que veoms es que toda la rama izquierda tiene la misma clase `MM`, así que podríamos ahorrarnos hasta tres subramas asignando a ese subárbol un nodo terminal que fuera `MM`. A la derecha se ve rápido que si lo primero que estudiáramos fuera la variable `ListPriceDiff` los cómputos serían más rápidos, ya que sólo de él depende que sea `MM` y no `CH`. Este árbol, en definitiva, no es en absoluto un buen resultado.
...
En la matriz de confusión lo primero que vemos es una tabla resumiendo los falsos positivos y los falsos negativos. Como vemos, es mucho más frecuente el error de clasificar las muestras `CH` como `MM`; esto no debe sorprendernos, ya que la gran mayoría de nodos terminales en el árbol tienen la última clase.
...
Como vemos, el mejor tamaño para este árbol es de `r I(arbol.cv$size[which(arbol.cv$dev == min(arbol.cv$dev))])`
...
arbol.n <- arbol.cv$size[which(arbol.cv$dev == min(arbol.cv$dev))]
...
La tasa de error es ahora de `r I((1-arbol.podado.conf$overall["Accuracy"]) * 100)`%, algo mejor que anteriormente. Aunque esta mejora no es muy importante, hay que notar que ahora el árbol es más compacto, legible y eficiente, así que hemos ganado tanto en eficiencia como en efectividad. En este caso la validación cruzada ha merecido la pena.
